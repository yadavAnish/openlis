...
abstract public class CSVColumnBuilder {
     ...
     * 
     */
	public CSVColumnBuilder(StatusOfSampleUtil.AnalysisStatus validStatusFilter) {
		...
		if (validStatusFilter != null) {
			this.validStatusId = StatusOfSampleUtil.getStatusID(validStatusFilter);
		}
	}
	...
	//It's actually 63 but UTF_8 makes 59 safer.  If 
	...
	private String trimToPostgresMaxColumnName(String name) {
		return name.length() > MAX_POSTGRES_COL_NAME ? name.substring(0, MAX_POSTGRES_COL_NAME) : name;
	}
	...
	public class CSVColumn {
		...
		public String translate(String value, String accessionNumber) throws Exception {
			...
			case SAMPLE_STATUS:
				OrderStatus orderStatus = StatusOfSampleUtil.getOrderStatusForID(value);
				if (orderStatus == null)
					return "?";
				switch (orderStatus) {
				case Entered:
